...
class ChineseOCRBreezper:
    ...
    def __init__(self, config):
        ...
        self.ocr_model.eval()
        print(f"✓ OCR 模型載入成功 (device: {self.device})")

        # 載入中文 tokenizer (使用適配器)
        print(f"載入中文 tokenizer 適配器...")
        tokenizer_path = getattr(self.config, 'ocr_tokenizer_path', self.config.ocr_model_path)
        self.ocr_tokenizer = ChineseCharTokenizerAdapter.from_pretrained(tokenizer_path)
        ...
        print(f"✓ 中文 tokenizer 載入成功")

        # 載入 LLM tokenizer 和模型
        print(f"載入 LLM: {self.config.llm_model_path}")
    ...
    def get_transcription(self, image_input, num_beams=5, ocr_prompt="", llm_prompt=""):
        ...
        Args:
            image_input: 圖像輸入 (路徑/numpy array/PIL Image)
        ...
        try:
            ...
            print(f"⚠️  無法使用 OCRImageTransform，使用標準預處理: {e}")
            # Fallback: 使用標準 TrOCR processor
    ...
    def _get_prefix_decoding_ids(self, ocr_prompt, llm_prompt):
        """獲取前綴解碼 IDs"""
        # TrOCR 使用 eos_token_id 作為 decoder_start_token_id
    ...
    def _decode_with_fusion(self, encoder_outputs, num_beams, ocr_prompt, llm_prompt, use_cache=None):
        ...
        while True:
            for beam in beams.list():
                ...
                # 將 OCR tokens 轉換為 bytes 以供 LLM 使用
